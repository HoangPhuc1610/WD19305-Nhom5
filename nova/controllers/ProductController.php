...
switch($action){
    ...
    case "editproduct":
        ...
        case "updateproduct":
            ...
            $category = $_POST['category'] ?? "";
            $status = $_POST['status'] ?? 1; // Mặc định là 1 (hiển thị)
            ...
            // Nếu có lỗi, hiển thị trang chỉnh sửa và dừng xử lý
            if (count($errors) > 0) {
                $products = getAllProducts();
                include "views/admin/products/edit.php";
                break;
            ...
            // Sau khi cập nhật thành công, chuyển hướng về trang sản phẩm
            header("Location: $baseurl/product");
            exit;
