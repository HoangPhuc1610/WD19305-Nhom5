...
<div class="ordermenu">
    ...
    <a href="<?= $baseurl ?>/ordertransit" class="active">Đơn hàng đang giao</a>
...
<form method="POST">
    <table border="1">
        ...
        foreach ($getOrderTransit as $transit) {
            // Lọc chi tiết đơn hàng tương ứng với order_id
            $orderDetailsForCurrentOrder = array_filter($orderdetails, function($orderdetail) use ($transit) {
                return $orderdetail['order_id'] == $transit['id'];
            });

            // Nếu chưa có đơn hàng trong mảng mergedOrders, tạo mới
            if (!isset($mergedOrders[$transit['id']])) {
                $mergedOrders[$transit['id']] = [
                    'customer' => $transit['customer'],
                    'products' => [],
                    'totalQuantity' => 0,
                    'totalPrice' => 0,
                    'date' => $transit['create_at'],
            ...
            foreach ($orderDetailsForCurrentOrder as $orderdetail) {
                ...
                $mergedOrders[$transit['id']]['products'][] = "{$title} (x{$quantity})";
                $mergedOrders[$transit['id']]['totalQuantity'] += $quantity;
                $mergedOrders[$transit['id']]['totalPrice'] += $price * $quantity;
            }
        }
