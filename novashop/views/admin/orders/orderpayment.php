...
<div class="ordermenu">
    <a href="<?= $baseurl ?>/order" >Tất cả đơn hàng</a>
    ...
    <a href="<?= $baseurl ?>/orderpayment"class="active">Đơn hàng được thanh toán</a>
</div>
...
<form method="POST">
    <table border="1">
        ...
        <?php 
        // Tạo một mảng để lưu đơn hàng đã gộp
        $mergedOrders = [];

        foreach ($getOrderPayment as $payment) {
            // Lọc chi tiết đơn hàng tương ứng với order_id
            $orderDetailsForCurrentOrder = array_filter($orderdetails, function($orderdetail) use ($payment) {
                return $orderdetail['order_id'] == $payment['id'];
            });

            // Nếu đã có order_id trong mảng mergedOrders, bỏ qua bước gộp mới
            if (!isset($mergedOrders[$payment['id']])) {
                $mergedOrders[$payment['id']] = [
                    'customer' => $payment['customer'],
                    'products' => [],
                    'totalQuantity' => 0,
                    'totalPrice' => 0,
                    'date' => $payment['create_at'],
            ...
            foreach ($orderDetailsForCurrentOrder as $orderdetail) {
                ...
                $mergedOrders[$payment['id']]['products'][] = "{$title} (x{$quantity})";
                $mergedOrders[$payment['id']]['totalQuantity'] += $quantity;
                $mergedOrders[$payment['id']]['totalPrice'] += $price * $quantity;
            }
        }
                ...
                <td>
                    <form method="POST">
                        <select name="status">
                            ...
                            <option value="4" <?= $order['status'] == 4 ? 'selected' : ''; ?>>Đơn hàng đã thanh toán</option>
                        </select>
