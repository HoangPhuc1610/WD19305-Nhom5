...
<div class="ordermenu">
    ...
    <a href="<?= $baseurl ?>/orderproces"class="active">Đơn hàng đang xử lý</a>
...
<form method="POST">
    <table border="1">
        ...
        foreach ($getProcessingOrders as $process) {
            // Lọc chi tiết đơn hàng tương ứng với order_id
            $orderDetailsForCurrentOrder = array_filter($orderdetails, function($orderdetail) use ($process) {
                return $orderdetail['order_id'] == $process['id'];
            });

            // Nếu đã có order_id trong mảng mergedOrders, bỏ qua bước gộp mới
            if (!isset($mergedOrders[$process['id']])) {
                $mergedOrders[$process['id']] = [
                    'customer' => $process['customer'],
                    'products' => [],
                    'totalQuantity' => 0,
                    'totalPrice' => 0,
                    'date' => $process['create_at'],
            ...
            foreach ($orderDetailsForCurrentOrder as $orderdetail) {
                ...
                $mergedOrders[$process['id']]['products'][] = "{$title} (x{$quantity})";
                $mergedOrders[$process['id']]['totalQuantity'] += $quantity;
                $mergedOrders[$process['id']]['totalPrice'] += $price * $quantity;
            }
        }
